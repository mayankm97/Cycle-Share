package com.example.cycleshare.auth.gsignin
...
class GoogleSignInViewModel @Inject constructor() : ViewModel() {
    ...
    private val _user = MutableStateFlow<User?>(null) // Initial state is null
    val user = _user.asStateFlow() // Expose as StateFlow

    fun signOut(navController: NavController) {
        FirebaseAuth.getInstance().signOut()
        _user.value = null // Reset the user data
        navController.navigate(Routes.signInScreen) { // Navigate back to sign-in screen
            popUpTo(0) // Clear the back stack
        }
    }
    // Function to handle Google Sign-In
    fun handleGoogleSignIn(googleSignInViewModel: GoogleSignInViewModel,context: Context, navController: NavController) {
    ...
    private suspend fun googleSignIn(context: Context): kotlinx.coroutines.flow.Flow<Result<AuthResult>> {
        ...
        return callbackFlow {
            try {
                ...
                //`MessageDigest` is a class in Java and Kotlin that is used to generate a hash value (or checksum) from input data.
                //Hashing the nonce adds an additional layer of security.
                // The `fold` function is a higher-order function that allows you to accumulate values in a collection (in this case, a byte array).
